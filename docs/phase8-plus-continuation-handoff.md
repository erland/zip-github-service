> **Current handoff r0139 / 1.0.0-rc.91:** Step 9.23 corrects the active browser title to `zip-GitHub` while preserving historical legacy naming. The implementation plan is reopened with Step 9.24 `NEXT` for explicit decisions on every blocking review entry, followed by Step 9.25 for conservative global cleanup of orphaned `zip-github/work-*` branches. No review/branch-cleanup behavior is implemented yet in rc.91.
