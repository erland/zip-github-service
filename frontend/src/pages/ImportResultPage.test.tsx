...
it('shows both next-ZIP and direct finish-work actions after commit', async () => {
  ...
  expect(await screen.findByDisplayValue(/Add explicit PR metadata/)).toHaveValue('- Keep Work open after PR\n- Add explicit PR metadata');
  expect(screen.getByLabelText('Titel')).toHaveValue('Improve ZIP review flow');
  ...
  expect(fetchMock).toHaveBeenCalledWith('/api/projects/p1/work/pull-request', expect.objectContaining({ method: 'POST', body: JSON.stringify({ title: 'Improve ZIP review flow', description: '- Keep Work open after PR\n- Add explicit PR metadata\n\nExtra context edited by the user.' }) }));
});
