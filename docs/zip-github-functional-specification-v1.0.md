...
| **Status**  | Utkast för genomförandeplanering                                              |
...
Rekommenderad modell: GitHub OAuth används för användarinloggning och en GitHub App-installation används för repositoryåtkomst. Identiteten och repositorybehörigheten är relaterade men separata.
...
4.  Användaren skapar ett projekt och väljer repository, jämförelsebranch och importbeteende.
...
7.  En GitHub-branch väljs, med projektets standard som förval.

8.  Användaren väljer ZIP-fil från enhetens filväljare.

9.  Appen laddar upp, validerar och packar säkert upp filen i temporär lagring.

10. Innehållet jämförs med ett oföränderligt commit-SHA för vald branch.

11. Användaren granskar sammanfattning, varningar, fillista och vid behov textdiffar.

12. Användaren väljer importbranch/pull request eller direkt commit där policy tillåter.

13. Användaren godkänner importen.

14. Appen skapar commit, pushar branch och öppnar normalt en pull request.

15. Resultatsidan visar GitHub-länkar och kan följa Actions/checkstatus.
...
| **ID**     | **Krav**           | **Beskrivning**                                                                                       | **Prioritet** |
|------------|--------------------|-------------------------------------------------------------------------------------------------------|---------------|
| **FR-C01** | Branchsnapshot     | Jämförelse ska ske mot ett commit-SHA som lösts från vald branch.                                     | Måste         |
...
| **FR-C04** | Fillista           | Påverkade sökvägar ska kunna granskas grupperat.                                                      | Måste         |
...
| **FR-C07** | Borttagningsskydd  | Borttagningar kräver uttrycklig policy och tydlig presentation.                                       | Måste         |
| **FR-C08** | Skyddade sökvägar  | Ändringar i skyddade sökvägar blockeras eller kräver särskilt godkännande.                            | Måste         |
| **FR-C09** | Hemlighetsvarning  | Troliga credentials och privata nycklar bör blockeras eller varnas för.                               | Bör           |
| **FR-C10** | Inaktuell bas      | Målbranchens SHA ska kontrolleras igen före skrivning.                                                | Måste         |

## 5.4 Commit och pull request

| **ID**     | **Krav**         | **Beskrivning**                                                          | **Prioritet** |
|------------|------------------|--------------------------------------------------------------------------|---------------|
| **FR-G01** | Godkännandespärr | Ingen GitHub-skrivning får ske före uttryckligt godkännande.             | Måste         |
| **FR-G02** | Importbranch     | Standardläget skapar en unik branch från jämförelsecommitten.            | Måste         |
| **FR-G03** | Atomisk commit   | Alla godkända förändringar ska normalt ingå i en commit.                 | Måste         |
| **FR-G04** | Metadata         | Commitmeddelandet bör referera projekt, import-ID, ZIP-namn och SHA-256. | Bör           |
| **FR-G05** | Push             | Branchen ska pushas med GitHub App-behörighet.                           | Måste         |
| **FR-G06** | Pull request     | Appen ska kunna skapa pull request till vald målbranch.                  | Måste         |
| **FR-G07** | Direkt commit    | Direkt commit tillåts endast enligt projektpolicy och branchskydd.       | Kan           |
| **FR-G08** | Resultatlänkar   | Branch-, commit- och PR-länkar ska sparas och visas.                     | Måste         |
...
## 6.1 Rekommenderad sökvägspolicy

Varje projekt bör definiera sökvägskategorier. Policy kan lagras i appen eller repositoryt; repositorylokal policy är bäst för portabilitet och granskning.
...
## 6.2 Borttagningssemantik

- MVP: filer som saknas i ZIP raderas inte.

- Senare mirror-läge: borttagning endast inom uttryckligt hanterade sökvägar.

- Skyddade sökvägar raderas aldrig genom normal import.

- Varje borttagning listas separat och kräver godkännande.

- Manifest kan bära base_commit och managed_paths.

## 6.3 Branchkonsistens

- Vald branch löses till commit-SHA före jämförelse.

- Importplanen byggs mot exakt denna commit.

- Före push kontrolleras att målbranchen inte oväntat flyttats.

- Vid förändring krävs ny jämförelse eller tydligt rapporterad divergens.

- ZIP-manifestets base_commit jämförs med vald branch och gammalt paket varnas/blockeras.

# 7. GitHub-integration

## 7.1 Rekommenderad autentiseringsmodell

GitHub OAuth används för att fastställa användarens identitet. En GitHub App-installation används för repositoryoperationer och begränsas till utvalda repositoryn och minsta nödvändiga behörigheter.
...
| **GH-P06** | Workflows     | Begär inte separat behörighet att ändra workflowfiler i MVP.       | Ingen         |
...
| **UI-05** | Förändringsgranskning | Sammanfattning, varningar, grupperad fillista och senare textdiff. | MVP           |
| **UI-06** | Godkännande           | Mål, PR-val, commitmeddelande och bekräftelse.                     | MVP           |
...
| **DM-06** | ImportPlan          | Förändringssammanfattning, varningar, sökvägar och godkänd snapshot. | Kärna         |
...
| **POST**  | /api/imports/{id}/approve   | Godkänn och genomför import.          |
...
| **NFR-S07** | Workflow-skydd    | .github/workflows blockeras som standard.                                      | Måste         |
...
- Jämförelse av tillagda och ändrade filer.

- Inga borttagningar i MVP.

- .github/\*\* skyddas.

- Sammanfattning och fillista.

- Importbranch, en commit och pull request.
...
25. AC-06: Granskningssidan listar alla tillagda och ändrade filer samt ignorerade/blockerade sökvägar.

26. AC-07: Ingen GitHub-skrivning sker före godkännande.

27. AC-08: Godkännande skapar en branch och en commit med exakt godkänd filuppsättning.

28. AC-09: En pull request skapas till vald målbranch.

29. AC-10: Resultatsidan länkar till commit, PR och GitHub Actions/checks.

30. AC-11: Temporär ZIP och arbetsyta städas enligt policy.

31. AC-12: Auditdata visar vem som importerade vilken checksumma till vilket repository och commit.
