...
export function ReviewFileTree({ entries, selectedPaths, onSelectedPathsChange, overridePaths, onOverridePathsChange, blockerDecisions = new Map(), onBlockerDecisionsChange, externalChangedPaths = new Set(), locked = false }: {
  entries: ImportPlanEntry[];
  ...
  onBlockerDecisionsChange?: (decisions: Map<string, BlockerDecision>) => void;
    ...
    <ul className="review-tree" aria-label="Filträd">
      {nodes.map((node) => (
        <TreeNode key={`${node.kind}-${node.path}`} node={node} selectedPaths={selectedPaths}
          ...
          onOverridePathsChange={onOverridePathsChange} blockerDecisions={blockerDecisions} onBlockerDecisionsChange={onBlockerDecisionsChange} externalChangedPaths={externalChangedPaths} locked={locked} level={1} />
      ))}
    </ul>
  );
}

function TreeNode({ node, selectedPaths, onSelectedPathsChange, overridePaths, onOverridePathsChange, blockerDecisions, onBlockerDecisionsChange, externalChangedPaths, locked, level }: {
  node: ReviewTreeNode;
  selectedPaths: ReadonlySet<string>;
  onSelectedPathsChange: (selectedPaths: Set<string>) => void;
  overridePaths: ReadonlySet<string>;
  ...
  if (node.kind === 'file') {
    return <FileTreeNode node={node} selectedPaths={selectedPaths} onSelectedPathsChange={onSelectedPathsChange}
      overridePaths={overridePaths} onOverridePathsChange={onOverridePathsChange} blockerDecisions={blockerDecisions} onBlockerDecisionsChange={onBlockerDecisionsChange} externalChangedPaths={externalChangedPaths} locked={locked} level={level} />;
  }
  return <DirectoryTreeNode node={node} selectedPaths={selectedPaths} onSelectedPathsChange={onSelectedPathsChange}
    overridePaths={overridePaths} onOverridePathsChange={onOverridePathsChange} blockerDecisions={blockerDecisions} onBlockerDecisionsChange={onBlockerDecisionsChange} externalChangedPaths={externalChangedPaths} locked={locked} level={level} />;
}

function DirectoryTreeNode({ node, selectedPaths, onSelectedPathsChange, overridePaths, onOverridePathsChange, blockerDecisions, onBlockerDecisionsChange, externalChangedPaths, locked, level }: {
        ...
        <ul className="review-tree__children">
          {node.children.map((child) => (
            <TreeNode key={`${child.kind}-${child.path}`} node={child} selectedPaths={selectedPaths}
              ...
              onOverridePathsChange={onOverridePathsChange} blockerDecisions={blockerDecisions} onBlockerDecisionsChange={onBlockerDecisionsChange} externalChangedPaths={externalChangedPaths} locked={locked} level={level + 1} />
          ))}
        </ul>
      )}
    </li>
...
function FileTreeNode({ node, selectedPaths, onSelectedPathsChange, overridePaths, onOverridePathsChange, blockerDecisions, onBlockerDecisionsChange, externalChangedPaths, locked, level }: {
  node: ReviewFileNode;
  selectedPaths: ReadonlySet<string>;
  onSelectedPathsChange: (selectedPaths: Set<string>) => void;
  overridePaths: ReadonlySet<string>;
  ...
  const hardBlocked = entry.blockerType === 'HARD_BLOCKED';
  ...
  const decision = blockerDecisions.get(entry.path);
  ...
  function toggleFile() {
    if (locked || !ordinarySelectable) return;
    const next = new Set(selectedPaths);
    if (checked) next.delete(entry.path); else next.add(entry.path);
    onSelectedPathsChange(next);
  }

  function decide(nextDecision: BlockerDecision) {
    if (locked || !onBlockerDecisionsChange) return;
    const decisions = new Map(blockerDecisions);
    const nextSelected = new Set(selectedPaths);
    const nextOverrides = new Set(overridePaths);
    decisions.set(entry.path, nextDecision);
    if (nextDecision === 'INCLUDE_OVERRIDE') {
    ...
    onBlockerDecisionsChange(decisions);
  ...
  return (
    <li className={`review-tree__item review-tree__item--file review-tree__item--${entry.status.toLowerCase()}`}>
      <div className="review-tree__row review-tree__row--file" style={{ '--tree-level': level } as CSSProperties}>
        ...
        {entry.status !== 'IGNORED' && <input type="checkbox" checked={checked} disabled={locked || !ordinarySelectable}
        ...
        <div className="review-tree__badges">
          ...
          {hardBlocked && <span className="file-status file-status--blocked">Hårt blockerad</span>}
          {overridable && <span className="file-status file-status--blocked">Kräver beslut</span>}
          {entry.modeChanged && <span className="file-status file-status--warning">Mode {entry.repositoryMode} → {entry.effectiveMode}</span>}
        </div>
      </div>
      {overridable && onBlockerDecisionsChange && <fieldset className="review-tree__override" style={{ '--tree-level': level } as CSSProperties} disabled={locked}>
        <legend>Beslut för blockerad förändring</legend>
        <label><input type="radio" name={`blocker-${entry.path}`} checked={decision === 'EXCLUDE'} onChange={() => decide('EXCLUDE')} /> Ta inte med</label>
        <label><input type="radio" name={`blocker-${entry.path}`} checked={decision === 'INCLUDE_OVERRIDE'} onChange={() => decide('INCLUDE_OVERRIDE')} /> Jag förstår risken – godkänn och ta med</label>
      </fieldset>}
      {hardBlocked && onBlockerDecisionsChange && <label className="review-tree__override" style={{ '--tree-level': level } as CSSProperties}>
        <input type="checkbox" checked={decision === 'ACKNOWLEDGE_EXCLUSION'} disabled={locked}
          onChange={(event) => {
            const decisions = new Map(blockerDecisions);
            if (event.target.checked) decisions.set(entry.path, 'ACKNOWLEDGE_EXCLUSION'); else decisions.delete(entry.path);
            onBlockerDecisionsChange(decisions);
          }} />
        <span>Jag har sett att denna hårt blockerade förändring inte kommer att tas med</span>
      </label>}
      {(entry.message || disabledReason || externalChanged) && <p className="review-tree__message" style={{ '--tree-level': level } as CSSProperties}>
