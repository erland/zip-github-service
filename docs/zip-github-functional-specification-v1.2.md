...
9.  Appen laddar upp och validerar ZIP-filen och fortsätter automatiskt med säker inventory, repositorysnapshot, jämförelse, policy och skapande av immutable importplan. Någon separat knapp för att “skapa granskningsplan” ska inte krävas i normalflödet.

10. Innehållet jämförs med ett oföränderligt commit-SHA för den aktuella basen och användaren förs automatiskt till granskningsvyn när planen är klar.
...
13. Överstyrbara blockerare, exempelvis faktiska förändringar under `.github/**` eller borttagningar enligt policy, är exkluderade som standard och kräver uttryckligt override-godkännande för att kunna inkluderas. En `UNCHANGED` fil kräver aldrig change-override enbart på grund av sin sökväg. Hårt blockerade poster, exempelvis `.git/**`, kan inte inkluderas men hindrar inte att övriga valda förändringar godkänns.

14. När användaren klickar “Godkänn valda förändringar” skapar/låser appen en immutable godkänd selection som binder plan-SHA, base commit, valda/exkluderade sökvägar och eventuella overrides.

15. Samma användaråtgärd fortsätter direkt med exakt diffverifiering, commit och push på projektets aktiva arbetsbranch. Ett separat normalt klick för “skapa commit” ska inte krävas.
...
| **FR-U09** | Automatisk plan    | Lyckad ZIP-uppladdning ska automatiskt fortsätta till inventory, snapshot, comparison, policy och immutable plan. | Måste |
| **FR-U10** | Återanvändbar ingestion | ZIP-streaming, storleksgräns, checksumma, filnamn och lagring ska kunna återanvändas av flera ingestion-kanaler. | Måste |
| **FR-U11** | Redan lagrad ZIP   | En säkert lagrad ZIP ska kunna promoveras/registreras som vanlig import utan en andra nätverksuppladdning. | Måste |
...
| **FR-C16** | Oförändrade skyddade paths | `UNCHANGED` ska inte kräva override enbart på grund av pathpolicy som `.github/**`; override gäller endast en faktisk repositoryförändring. | Måste |
...
| **FR-G11** | Ett-klicksleverans | “Godkänn valda förändringar” ska i normalflödet registrera approval och direkt fortsätta till exakt commit/push utan separat commit-knapp. | Måste |
...
- faktiska `ADDED`, `MODIFIED` eller `WOULD_DELETE` under `.github/**`, inklusive workflows,
- borttagningar,
...
`ADDED` och `MODIFIED` är markerade som standard. `WOULD_DELETE` är synligt men exkluderat tills användaren explicit tillåter borttagningen. `UNCHANGED` och transportbrus behöver normalt inte vara valbara. Pathbaserade blockerare som `.github/**` får inte omklassificera en `UNCHANGED` post till en förändring som kräver override; eftersom ingen repositoryskrivning sker finns ingen sådan change-risk att godkänna.
...
## 6.3 Återanvändbar ingestion och framtida staging

Säker ZIP-mottagning ska vara separerad från den användarägda importorkestreringen. En ingestion ska kunna producera ett neutralt `StoredUpload` med temporär storage-referens, filnamn, storlek och SHA-256. En vanlig webbuppladdning skapar därefter en vanlig `ImportSession` från detta resultat.

Samma integrationspunkt ska kunna användas av en framtida kortlivad `StagingImport` (exempelvis iOS Shortcut): staging-lagret får endast ta emot/lagra ZIP och därefter, efter vanlig autentisering/claim, promovera den redan lagrade ZIP-filen till en vanlig användarägd import. Från och med den punkten måste befintlig inventory-, snapshot-, comparison-, policy-, plan-, selection-, workspace- och delivery-pipeline användas utan parallell implementation.

Importkällan får auditklassificeras men ska inte ändra säkerhetspolicy, plan-digest eller selection-semantik. Capability-/claim-tokenvärden får aldrig sparas som vanlig sourceReference/audittext.
...
| **UI-04** | Uppladdning           | ZIP-väljare, förlopp och validering; efter lyckad upload fortsätter appen automatiskt till plan och granskningsvy. | MVP/RC |
...
| **UI-06** | Godkännande           | Ett klick bekräftar exakt selection/overrides/commitmetadata och fortsätter därefter direkt till commit/push. | MVP/RC |
...
| **DM-12** | StoredUpload        | Neutralt resultat av säker ZIP-ingestion: temporär storage-referens, filnamn, storlek, SHA-256 och retention. | Kärna |
| **DM-13** | ImportSource        | Icke-hemlig klassificering av hur ZIP:en nådde en vanlig import, exempelvis `WEB_UPLOAD` eller `STAGING_IMPORT`. | Kärna |
...
34. AC-15: Efter lyckad ZIP-upload skapas granskningsplanen automatiskt och användaren förs till granskningsvyn utan separat “Skapa granskningsplan”-klick.

35. AC-16: Ett klick på “Godkänn valda förändringar” leder i normalfallet hela vägen till commit/resultat; eventuell separat delivery-åtgärd används endast för återhämtning efter fel.

36. AC-17: En oförändrad fil under `.github/**` kräver inget override, medan en ny, ändrad eller borttagen fil under `.github/**` fortfarande kräver explicit auditerat override.

37. AC-18: Samma säkra ZIP-ingestion och vanliga importpipeline kan användas för både webbuppladdad och redan lagrad ZIP utan andra nätverksuppladdning.
